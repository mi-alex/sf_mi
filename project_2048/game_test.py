...
def eval_up_prob(test_board) -> bool: # оценка вероятности вынужденного up
    ...
    for attempt in range(10):
        ...
        if not can_move(try_board, 'down') and not can_move(try_board, 'left') and not can_move(try_board, 'right'):
            danger = True
        test_board = []
        for line_num in range(4):
            line_to_add = []
...
def decision_maker(board, echo) -> str: # принятие решения о ходе
    ...
    if can_down:
        ...
        if can_move(try_board_down, 'right'):
            r_score = estimate_board(try_board_down, 'right')
        if can_move(try_board_down, 'left'):
            l_score = estimate_board(try_board_down, 'left')
...
test_board = [[0,0,0,4],[2,4,2,4],[2,16,128,16],[512,64,256,128]]
...
Step: 515
#Go down because it is better: down 13, left 11, right 0

decision_maker(test_board, True)
